 ...
 *
 */
(function(a){a.fn.slippy=function(r){var w={speed:500,easing:"swing",auto:false,autoStart:true,pauseOnHover:true,delayStart:0,pause:3000,startingSlide:1,delayUntilEvent:false,infiniteWidth:false,widthSpeed:400,minWidth:960,minHeight:0,widthEasing:"swing",autoHeight:false,heightSpeed:1000,heightEasing:"swing",onBeforeSlide:function(){},onAfterSlide:function(){},onLastSlide:function(){},onFirstSlide:function(){},onNextSlide:function(){},onPrevSlide:function(){},onInitComplete:function(){}};r=a.extend(w,r);var n=this;var s=a(this);var e=s.children();var L=a(window);var p=r.startingSlide;var u=e.length;var d,h,j,z,f,l,o,E,x,i,k,N,g,b,Q,q,D,I,K,J,m,t,y,C,v=false,H,O,G=false;this.getCurrentSlide=function(){return p};this.startSlippy=function(){if(!i||v){return}O=false;G=false;H=window.setTimeout(function(){n.nextSlide()},b)};this.stopSlippy=function(){G=true;window.clearTimeout(H)};this.resumeSlippy=function(){G=false;n.startSlippy()};this.nextSlide=function(){if(G){return}var R=p+1;if(R>u){R=1}n.gotoSlide(R)};this.prevSlide=function(){if(G){return}var R=p-1;if(R<1){R=u}n.gotoSlide(R)};this.gotoSlide=function(R,V){window.clearTimeout(H);if(!G||V){var U=e.eq(R-1);C.removeClass("selected").data("isSelected",false);C.eq(R-1).addClass("selected").data("isSelected",true);p=R;r.onBeforeSlide();F(U);var T=-(j*(p-1));var S=0;if(G){S=J}G=true;h.delay(S).animate({left:T},E,I,function(){G=false;A(U);n.startSlippy();r.onAfterSlide()})}};function M(){s.wrapInner('<div class="wrapper" />');h=s.children(".wrapper");f=L.width();l=r.minWidth;o=(r.minHeight<=0)?s.height():r.minHeight;E=r.speed;x=r.easing;i=r.auto;k=r.autoStart;N=r.pauseOnHover;g=r.delayStart;b=r.pause;O=r.delayUntilEvent;Q=r.startingSlide;q=r.infiniteWidth;D=r.widthSpeed;I=r.widthEasing;K=r.autoHeight;J=r.heightSpeed;m=r.heightEasing;s.css({overflow:"hidden",width:"100%",height:o}).addClass("slippy");h.css({position:"absolute",width:"999990px",top:"0px",left:"0px","z-index":"1"});e.css({position:"relative",width:"1600px",margin:"0px",padding:"0px","float":"left"});e.children(".background").css({position:"absolute","z-index":1,width:"100%",height:o,"background-position":"top left","background-repeat":"repeat-x"});e.children(".content").css({position:"relative",width:l,margin:"0px auto","z-index":10});var R='<div class="slippy-pager">';for(var S=1;S<=u;S++){R+='<a href="" class="slippy-pager-link">'+S+"</a>"}R+="</div>";s.append(R);y=s.find(".slippy-pager");C=y.children(".slippy-pager-link");C.each(function(T){a(this).click(function(U){n.gotoSlide(T+1,true);U.preventDefault()}).hover(function(){if(!a(this).data("isSelected")){a(this).addClass("selected")}},function(){if(!a(this).data("isSelected")){a(this).removeClass("selected")}})});if(q){if(!a.event.special.debouncedresize){throw"Requires jquery.debouncedresize.js. Get it here: https://github.com/louisremi/jquery-smartresize"}L.on("debouncedresize",function(T){B()});B(0)}P();c(Q);r.onInitComplete();if(k&&!O){n.startSlippy()}}function P(){if(N){h.hover(function(){if(i&&!O){v=true;n.stopSlippy()}},function(){if(i&&!O){v=false;n.startSlippy()}})}else{s.unbind("hover")}}function B(R){f=L.width();var V=l;var U=0;var T=1600;var S=(R===0)?R:D;if(f>V){V=f;U=-(f*(n.getCurrentSlide()-1))}else{U=-(V*(n.getCurrentSlide()-1))}if(f>T){T=f}s.animate({width:V},S,I);h.animate({left:U},S,I);e.animate({width:V},S,I);z=U;j=V}function F(T){if(!K){return}var S=s.outerHeight();var R=a(T).outerHeight();if(S!==R&&R<=o){G=true;s.animate({height:o},J,m)}}function A(T){if(!K){return}var S=s.outerHeight();var R=a(T).outerHeight();if(S!==R&&R>o){s.animate({height:R},J,m)}}function c(S){var V=e.eq(S-1);C.removeClass("selected").eq(S-1).addClass("selected");p=S;var U=-(j*(p-1));h.css({left:U});var T=s.outerHeight();var R=V.outerHeight();if(T!==R&&R>o){s.css({height:R})}}this.each(function(){if(a(this).children().length>0){M()}});return this}})(jQuery);
/*!
 ...
 * });
*/
(function(a,b){var c=function(a,b,c){var d;return function e(){function g(){if(!c)a.apply(e,f);d=null}var e=this,f=arguments;if(d)clearTimeout(d);else if(c)a.apply(e,f);d=setTimeout(g,b||100)}};jQuery.fn[b]=function(a){return a?this.bind("resize",c(a)):this.trigger(b)}})(jQuery,"smartresize");
...
d.initPlayer(a,b);return a};d.getPkg=function(a){a=a.data;var b=a.data("opts.tubeplayer");return{$player:a,opts:b,ytplayer:d.ytplayers[b.playerID]}};d.iframeReady=function(a){d.inits.push(function(){new YT.Player(a.playerID,{videoId:a.initialVideo,width:a.width,height:a.height,playerVars:{autoplay:a.autoPlay?1:0,autohide:a.autoHide,controls:a.showControls?1:0,rel:a.showRelated?1:0,fs:a.allowFullScreen?1:0,wmode:a.wmode,showinfo:a.showinfo?1:0,modestbranding:a.modestbranding?1:0,start:a.start,
...
d.initFlashPlayerFN=function(a){return function(){var b=["//www.youtube.com/v/"];b.push(a.initialVideo);b.push("?fs="+(a.allowFullScreen?1:0));b.push("&enablejsapi=1&version=3");b.push("&playerapiid="+a.playerID);b.push("&rel= "+(a.showRelated?1:0));b.push("&autoplay="+(a.autoPlay?1:0));b.push("&autohide="+(a.autoHide));b.push("&controls="+(a.showControls?1:0));b.push("&showinfo="+(a.showinfo?1:0));b.push("&modestbranding="+(a.modestbranding?1:0));b.push("&start="+a.start);b.push("&theme="+a.theme);
...
/*!
 * simple jQuery preloading plugin
 *
 * use
 * $(['img1.jpg', 'img2.jpg', 'img3.jpg']).preloadImages(function(){
 *     // preload complete, do something here
 * });
 */
Array.prototype.remove=function(c){for(var b=0,a=this.length;b<a;b++){if(this[b]==c){this.splice(b,1);}}};$.fn.preloadImages=function(b){var a=this.toArray();this.each(function(){$("<img>").attr({src:this}).load(function(){a.remove($(this).attr("src"));if(a.length==0){b();}});});};
/*
 * Date Format 1.2.3
